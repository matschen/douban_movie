...
// @version      0.1
// @description  将豆瓣已看电影导出为csv文件。启用本脚本，进入豆瓣个人页面后，在『我看』部分会有一链接『导出看过电影』，点击即可。无需登录，支持导出任意用户已看电影。
// @author       KiseXu
...
(function() {
    ...
    if (location.href.indexOf('//www.douban.com/') > -1) {
        ...
        var export_link = 'https://movie.douban.com/people/' + people + '/collect?start=0&sort=time&rating=all&filter=all&mode=list&export=1';
        $('#movie .pl a:last').after('&nbsp;·&nbsp;<a href="'+export_link+'">导出看过电影</a>')
    }
    ...
    function getCurrentPageList() {
        ...
        $('li.item').each(function(index) {
            items[index] = {
                title: $(this).find('a').text().replace(/修改删除/, '').trim(),
    ...
    function getPage() {
        ...
        db.version(1).stores({
            items: `++id, title, rating, date, link`
        });
    ...
    function exportAll() {
        ...
        db.items.orderBy('date').toArray().then(function(all){
            ...
            JSonToCSV.setDataConver({
                ...
                fileName: 'movie',
                columns: {
                    title: ['片名', '个人评分', '打分日期', '影片链接'],
                    key: ['title', 'rating', 'date', 'link']
                }
            });
            db.delete();
        });
